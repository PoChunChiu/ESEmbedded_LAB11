...
void svc_handler_c(uint32_t lr_v, uint32_t msp_bp)
{
	...
	if (lr_v&0b100) //Test bit 2 of EXC_RETURN
	{
		stack_frame_ptr = (unsigned int)read_psp(); //if 1, stacking used PSP
		printf("[SVC Handler] Stacking used PSP: 0x%X \r\n\n", (unsigned int)stack_frame_ptr);
	}
	else
	{
		stack_frame_ptr = (unsigned int)read_msp(); //if 0, stacking used MSP
